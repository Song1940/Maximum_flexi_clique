...
// to isolate the contribution of connectivity-preserving (CP) branching.
//
// Both baselines find the EXACT maximum flexi-clique using only the basic pruning
// shared with CP — Rule 1 (adjusted-degree) + the size bound + the FPA
...
// a solution (CP never needs this).  Rule 2 (diameter) and the width-based pivot are
...
struct BaselineSolver {
    ...
    // on candidates, cascade-filter), the size bound, and the incumbent size
    ...
    bool prune(const std::vector<int>& S, std::vector<int>& cand,
        ...
        // Size bound: |S| must not exceed F_max = floor((min adj-deg + 1)^{1/tau}).
        if (!S.empty()) {
            int md = INT_MAX;
...
#ifndef NO_WARMSTART
    // Lower bound IDENTICAL to CP (eba_cum): best of FPA / NPA, then Modified
