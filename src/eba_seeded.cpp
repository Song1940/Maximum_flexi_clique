...
// ============================================================
// eba_seeded.cpp — seeded exact search: maximum tau-flexi-clique containing a
// given query vertex within a local region (used by the case-study solver qcs).
// ============================================================
...
struct EBAState {
    ...
    // Incremental S-membership and S-internal degree.
    ...
    // Global degree pruning: iteratively remove nodes whose global degree falls below theta.
    ...
    // Follower cascade: simulate removing v and cascade degree drops.
    ...
    void recurse(std::vector<int>&  S,
        ...
        for (int idx = 0; idx < (int)R.size(); ++idx) {
            ...
            // Maintain deg_in_S incrementally.
            in_S[v] = true;
            for (int u : G.adj[v])
            ...
            // min_adj: recomputed after S.push_back(v) and follower exclusions.
